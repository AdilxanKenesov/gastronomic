...
class RestaurantBloc extends Bloc<RestaurantEvent, RestaurantState> {
  ...
  Future<void> _onLoadTopRestaurants(
    LoadTopRestaurants event,
    Emitter<RestaurantState> emit,
  ) async {
    emit(RestaurantLoading());

    try {
      final restaurants = await _restaurantService.getTopRestaurantsByCategory(
        categoryId: event.categoryId,
        limit: event.limit,
        language: event.language,
      );
  ...
  Future<void> _onLoadNearestRestaurants(
    LoadNearestRestaurants event,
    Emitter<RestaurantState> emit,
  ) async {
    emit(RestaurantLoading());

    try {
      final restaurants = await _restaurantService.getNearestRestaurants(
        latitude: event.latitude,
        longitude: event.longitude,
        language: event.language,
      );
